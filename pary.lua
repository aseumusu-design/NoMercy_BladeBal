--[[
=========================================================================
    NO MERCY HUB V3.5 + R6 TRUE FE INVISIBLE (ANTI-DEATH FIX)
    - Fix: Gak mati lagi (pindah ke X/Z 999999, bukan Y -500)
    - Fix: Gerak bebas WASD + Space + Shift (speed 90, smooth)
    - Fix: Anti-death (health lock, ragdoll off, breakjoints off)
=========================================================================
]]
...
local SavedBreakJoints = true

-- PINDAH KE X/Z JAUH biar gak kena kill boundary (Y = aman)
local HIDE_POS = Vector3.new(999999, 50, 999999)
...
-- ==================== GHOST / INVISIBLE SYSTEM (ANTI-DEATH FIX) ====================
local function StopGhost()
    ...
    if char then
        ...
        if hum then
            ...
            hum.BreakJointsOnDeath = SavedBreakJoints
            -- Re-enable states
            pcall(function()
...
local function StartGhost()
    ...
    -- Lock health tiap frame (biar gak mati kena damage/void)
    GhostHealthConn = RunService.Heartbeat:Connect(function()
        if hum and hum.Parent then
            hum.Health = math.huge
            hum.PlatformStand = true  -- Biar gak jatuh/stand up sendiri
        end
    end)
    ...
    -- Controller part (ini yang lu gerakin bebas)
    ...
    GhostMoveConn = RunService.RenderStepped:Connect(function(dt)
        ...
        -- Sprint (LeftControl buat turbo)
        local speed = 90
        if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then speed = 150 end
    ...
    -- ===== SYNC LOOP: Real body ke hide pos, clone ikut controller =====
    GhostSyncConn = RunService.Heartbeat:Connect(function()
        if not Config.GhostEnabled then return end
        
        -- Real body di-hide jauh di X/Z (bukan Y, biar gak mati)
        local realChar = LocalPlayer.Character
        if realChar then
            local realHRP = realChar:FindFirstChild("HumanoidRootPart")
            if realHRP then
                realHRP.CFrame = CFrame.new(HIDE_POS)
                realHRP.Velocity = Vector3.new(0, 0, 0)
                realHRP.RotVelocity = Vector3.new(0, 0, 0)
            end
        end
        
        -- Clone ikut controller
        if GhostClone and GhostClone.Parent and GhostController and GhostController.Parent then
            pcall(function()
...
Title.Text = "NO MERCY HUB V3.5 — ANTI-DEATH FIX"
...
ghostHint.Text = "Real body hidden at X/Z=999999 (safe, no death).\nYou control ghost clone (50% visible).\nWASD = Move | Space = Up | Shift = Down\nCtrl = Turbo Speed | Anti-Death Active!"
...
print("✅ NO MERCY HUB V3.5 — Anti-Death R6 Ghost Loaded!")

...
local function BuildShimOrion(getHolder, ICON, TweenService, LocalPlayer, UserInputService)
    ...
    function OrionLib:MakeWindow(cfg)
        ...
        function window:MakeTab(tcfg)
            ...
            -- Auto-select the first tab so content is visible immediately.
            if #tabs == 1 then showTab(page) end
...
-- ############################################################
-- ## ERROR DISPLAY UTILITY (shows errors on-screen) ##########
-- ############################################################
-- If the script crashes before the UI is built, this function
-- creates a visible error banner so the user can see WHAT went
-- wrong instead of a blank screen.
getgenv().NoMercy_ShowError = function(msg)
    pcall(function()
        local holder
        if gethui then
        ...
        -- Remove old error gui
        local old = holder:FindFirstChild("NoMercyError")
        if old then old:Destroy() end

        local gui = Instance.new("ScreenGui")
        gui.Name = "NoMercyError"
        gui.ResetOnSpawn = false
        gui.IgnoreGuiInset = true
        gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        gui.Parent = holder
        ...
        banner.Position = UDim2.new(0.5, -200, 0.5, -60)
        banner.BackgroundColor3 = Color3.fromRGB(40, 20, 20)
        banner.BorderSizePixel = 2
        banner.BorderColor3 = Color3.fromRGB(255, 60, 60)
        banner.Parent = gui
        ...
        msgLabel.Position = UDim2.new(0, 12, 0, 40)
        msgLabel.BackgroundTransparency = 1
        msgLabel.Text = tostring(msg):sub(1, 200)
        msgLabel.TextColor3 = Color3.fromRGB(255, 220, 220)
        ...
        closeBtn.Position = UDim2.new(1, -72, 1, -32)
        closeBtn.BackgroundColor3 = Color3.fromRGB(180, 40, 40)
        closeBtn.Text = "Close"
        closeBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
        closeBtn.Font = Enum.Font.GothamBold
        ...
        print("[NO MERCY ERROR] " .. tostring(msg))
    end)
end
...
-- ============================================================
--  SERVICES (all pcall-protected — a missing/restricted service
--  must NEVER halt the script before the UI is built)
-- ============================================================
...
pcall(ShowWelcomeIntro) -- protected: tween/animation must never halt UI build

-- ============================================================
--  ORION LIBRARY LOAD — robust (retry + multi-URL + shim fallback)
--  On MOBILE (Delta etc.) we SKIP the remote loadstring entirely
--  and use the self-contained shim, because remote OrionLib can
--  have PC-only behavior that breaks on mobile. On PC we try the
--  real OrionLib first, then fall back to the shim.
-- ============================================================
local function tryLoadOrion()
    -- Mobile: always use shim (remote OrionLib is PC-oriented).
    if isMobile then
        return nil
    end
    local urls = {
...
print("[NO MERCY] OrionLib loaded (remote or shim fallback). isMobile=" .. tostring(isMobile))

local onCloseRequest

-- Protected Window creation: if MakeWindow throws, fall back to shim.
local Window
...
if not okWindow or not Window then
    print("[NO MERCY] MakeWindow failed (" .. tostring(errWindow) .. ") — using shim fallback.")
    -- Force the shim and retry.
    local buildShim = getgenv().NoMercy_BuildShimOrion
    if buildShim then
        OrionLib = buildShim(GetHolder, ICON, TweenService, LocalPlayer, UserInputService)
        pcall(function()
...
pcall(setupAntiFail) -- protected: hookmetamethod may be unsupported on some executors
...
pcall(ListenParryResult) -- protected: remote structure may differ
...
-- Boot: protected so a failure here can NEVER halt the script
-- before Part 13 builds the UI.
task.spawn(function()
    pcall(function()
        for _, p in ipairs(Players:GetPlayers()) do
            pcall(SetupPlayer, p)
        end
        Players.PlayerAdded:Connect(function(p) pcall(SetupPlayer, p) end)
    end)
end)

-- Parry keybind (safe connect; callback already pcall-guarded)
pcall(function()
...
-- Boot: protected RenderStepped connect (body pcall-guarded)
pcall(function()
    RunService.RenderStepped:Connect(function()
...
-- (protected connect so it can't halt UI build)
pcall(function()
...
-- Auto-rebind God Mode on character respawn (protected connect)
pcall(function()
    LocalPlayer.CharacterAdded:Connect(function(char)
        task.wait(0.5)
...
pcall(veil_setupInterceptor) -- protected: hookmetamethod may be unsupported
...
end) -- close pcall wrapper for AUTO VAULT + AUTO PALLET SLIDE heartbeat
...
-- (PROTECTED so a failure here can NEVER halt the script
--  before Part 13 builds the UI tabs.)
task.spawn(function()
    pcall(function()
        for _, player in ipairs(Players:GetPlayers()) do pcall(IYAN_WatchPlayer, player) end
        table.insert(IYAN_Connections, Players.PlayerAdded:Connect(function(p) pcall(IYAN_WatchPlayer, p) end))
        table.insert(IYAN_Connections, Players.PlayerRemoving:Connect(function(p) pcall(IYAN_UnwatchPlayer, p) end))
...
-- ============================================================
-- Safety: if Window is nil (MakeWindow failed + shim fallback failed),
-- build an absolute last-resort minimal window so we at least show SOMETHING.
if not Window then
    pcall(function()
        local buildShim = getgenv().NoMercy_BuildShimOrion
        if buildShim then
            ...
            Window = OrionLib:MakeWindow({
                ...
                ConfigFolder = "NoMercyViolence", IntroEnabled = false,
                Icon = ICON.Logo, CloseCallback = function() if onCloseRequest then onCloseRequest() end end,
            })
        end
    end)
end
if not Window then
    -- ULTRA last resort: barebones inline window
    pcall(function()
        local holder = GetHolder()
        local gui = Instance.new("ScreenGui"); gui.Name = "MarV"; gui.ResetOnSpawn = false
        gui.IgnoreGuiInset = true; gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        gui.Parent = holder
        OrionLib = {}
        function OrionLib:MakeNotification(d) end
        function OrionLib:Destroy() pcall(function() gui:Destroy() end) end
        function OrionLib:MakeWindow() return OrionLib._win end
        local w = {}
        OrionLib._win = w
        function w:MakeTab(tcfg)
            local t = {}
            function t:AddSection(scfg)
                local s = {}
                function s:AddLabel(txt)
                    local lbl = Instance.new("TextLabel")
                    lbl.Size = UDim2.new(1, -20, 0, 20); lbl.Position = UDim2.new(0, 10, 0, 0)
                    lbl.BackgroundTransparency = 1; lbl.Text = txt or ""
                    lbl.TextColor3 = Color3.fromRGB(240,240,240); lbl.Font = Enum.Font.Gotham
                    lbl.TextSize = 12; lbl.TextXAlignment = Enum.TextXAlignment.Left
                    lbl.Parent = gui; return lbl
                end
                function s:AddToggle(c) if c.Callback then pcall(c.Callback, c.Default or false) end return {Value = c.Default or false} end
                function s:AddButton(c) return {} end
                function s:AddSlider(c) return {Value = c.Default or 0} end
                function s:AddDropdown(c) return {Value = c.Default or ""} end
                function s:AddColorPicker(c) return {} end
                return s
            end
            return t
        end
...
if not Window then
    getgenv().NoMercy_ShowError("FATAL: Could not create UI Window. Please report this.")
    error("[NO MERCY] Window creation failed completely")
end
...
print("[NO MERCY] UI built successfully — all 10 tabs created.")

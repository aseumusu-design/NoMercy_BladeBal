--[[
=========================================================================
    NO MERCY HUB V3.5 + FE INVISIBLE (PART REPLACEMENT)
    + CAMERA FOLLOW CLONE TOGGLE
    - Real body: Transparency=1 (tetap di posisi)
    - Ghost clone: Transparan 0.5, sync per-part tiap frame
    - Camera: Bisa ikut badan asli ATAU clone (toggle)
...
-- ==================== INVISIBLE SYSTEM (PART REPLACEMENT) ====================
...
local function UpdateCameraSubject()
    ...
    if Config.CameraFollowClone and GhostClone and GhostClone.Parent then
        local clonePart = GhostClone:FindFirstChild("Head") or GhostClone:FindFirstChild("HumanoidRootPart")
        if clonePart then
            Camera.CameraSubject = clonePart
        end
    else
        local char = LocalPlayer.Character
        if char then
...
local invMainDot = AddToggle(InvTab, "FE Invisible (Part Replace)", false, function(v)
    if v then EnableInvisible() else DisableInvisible() end
end)

local camDot = AddToggle(InvTab, "Camera Follow Clone", false, function(v)
    Config.CameraFollowClone = v
    UpdateCameraSubject()
end)
...
invHint.Text = "Real body hidden (Transparency=1).\nGhost clone 50% visible, sync per-part.\nCamera Follow Clone: ON = kamera ikut clone\nOFF = kamera ikut badan asli (default)\nHotkey: G"
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Enum.KeyCode.G then
        if Config.Invisible then
            DisableInvisible()
            invMainDot.BackgroundColor3 = Color3.fromRGB(60, 60, 75)
        else
            EnableInvisible()
            invMainDot.BackgroundColor3 = THEME.Green
        end
    end
end)
...
print("✅ NO MERCY HUB V3.5 — FE Invisible + Camera Follow Clone Loaded!")

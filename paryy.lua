--[[
=========================================================================
    NO MERCY HUB V3.5 + TRUE FE INVISIBLE & NOCLIP
    - Aimbot + Laser + FOV (original)
    - True FE Invisible (body di void -500, control part lokal)
    - NoClip
    - Aimbot kompatibel saat invisible (fire & laser dari controller)
=========================================================================
]]
...
local Config = {
    ...
    -- [MERGED] Invisible & NoClip config
    Invisible     = false,
    NoClip        = false,
}
...
-- ==================== [MERGED] INVISIBLE & NOCLIP STATE ====================
local invController = nil
local invBodyPos = nil
local invBodyGyro = nil
local invMoveConnection = nil
...
-- ==================== [MERGED] FIRE WEAPON (invisible aware) ====================
local function FireWeapon(targetPos)
    ...
    if gun and remote then
        local from
        -- [MERGED] Kalau invisible, tembak dari posisi controller (bukan body asli di -500)
        if Config.Invisible and invController and invController.Parent then
            from = invController.Position
        else
            from = gun:IsA("BasePart") and gun.Position or Camera.CFrame.Position
        end
...
-- ==================== [MERGED] INVISIBLE SYSTEM ====================
local function cleanupInvisible()
    if invMoveConnection then invMoveConnection:Disconnect() invMoveConnection = nil end
    if invController then invController:Destroy() invController = nil end
    if invBodyPos then invBodyPos:Destroy() invBodyPos = nil end
    if invBodyGyro then invBodyGyro:Destroy() invBodyGyro = nil end
...
local function enableInvisible()
    ...
    invController = Instance.new("Part")
    invController.Name = "LocalController"
    invController.Size = Vector3.new(2, 2, 1)
    invController.Transparency = 1
    invController.CanCollide = false
    invController.Anchored = true
    invController.CFrame = hrp.CFrame
    invController.Parent = Workspace
    
    hrp.Velocity = Vector3.new(0, 0, 0)
    hrp.CFrame = CFrame.new(HIDE_POS)
    
    invBodyPos = Instance.new("BodyPosition")
    invBodyPos.MaxForce = Vector3.new(9e9, 9e9, 9e9)
    invBodyPos.Position = HIDE_POS
    invBodyPos.P = 1e6
    invBodyPos.D = 1e4
    invBodyPos.Parent = hrp
    
    invBodyGyro = Instance.new("BodyGyro")
    invBodyGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9)
    invBodyGyro.CFrame = CFrame.new(HIDE_POS)
    invBodyGyro.P = 1e6
    invBodyGyro.D = 1e4
    invBodyGyro.Parent = hrp
    
    Camera.CameraSubject = invController
    
    invMoveConnection = RunService.RenderStepped:Connect(function(dt)
        if not invController or not invController.Parent then return end
        ...
        if moveDir.Magnitude > 0 then
            invController.CFrame += moveDir.Unit * 70 * dt
        end
    end)
end
...
local function disableInvisible()
    ...
    local returnCF = invController and invController.CFrame
    cleanupInvisible()
...
-- ==================== MAIN AIMBOT LOOP (MODIFIED LASER ORIGIN) ====================
    ...
    if CurrentTarget then
        ...
        if pos then
            if Config.LaserEnabled then
                -- [MERGED] Laser origin dari controller kalau invisible
                local from
                if Config.Invisible and invController and invController.Parent then
                    from = invController.Position
                else
                    ...
                    from = (gun and gun.Position) or Camera.CFrame.Position
                end
...
Title.Text = "NO MERCY HUB V3.5 — INVISIBLE EDITION"
...
local InvisTab = CreateTab("Invisibility")   -- [MERGED]
local NoclipTab = CreateTab("NoClip")        -- [MERGED]
...
-- ==================== [MERGED] INVISIBILITY TAB CONTENT ====================
AddToggle(InvisTab, "True FE Invisible", false, function(v)
    if v then enableInvisible() else disableInvisible() end
end)

local invisHint = Instance.new("TextLabel", InvisTab)
invisHint.Size = UDim2.new(1, 0, 0, 50)
invisHint.BackgroundTransparency = 1
invisHint.Text = "Body asli di-hide di void (Y=-500).\nKamu control part lokal. Orang lain gak lihat samsek.\nWASD jalan | Space naik | Shift turun"
invisHint.TextColor3 = THEME.TextDim
invisHint.TextSize = 10
invisHint.Font = Enum.Font.Gotham
invisHint.TextWrapped = true
invisHint.TextXAlignment = Enum.TextXAlignment.Left

-- ==================== [MERGED] NOCLIP TAB CONTENT ====================
...
noclipHint.Text = "Bisa lewat tembok, bangunan, tanah.\nAuto-disable collision semua part karakter."
...
-- ==================== [MERGED] CHARACTER EVENTS ====================
LocalPlayer.CharacterRemoving:Connect(function()
    if invController then
        invController:Destroy()
        invController = nil
    end
    if invMoveConnection then
        invMoveConnection:Disconnect()
        invMoveConnection = nil
    end
end)
...
print("✅ NO MERCY HUB V3.5 + True FE Invisible & NoClip Loaded!")
